...
1. **Table data losing meaning during PDF extraction.** During initial testing with pdfplumber, table cells were extracted as meaningless letter sequences (e.g. "N N N T T X X X") because pdfplumber reads text left-to-right without understanding table structure. Dollar signs, percentages, and numeric symbols were also dropped due to font encoding issues. Since this RAG is heavily dependent on numeric data (award amounts, funding figures, enrollment percentages), this is a critical problem. To address this, the ingestion pipeline was switched from pdfplumber to **docling** (IBM, 2024), which understands document structure and converts tables into clean markdown before chunking — preserving the relationships between row headers, column headers, and cell values.
